...
class ChargePay(APIView):
    ...
    def post(self,request):
        ...
        body = str(body)
        try:
            recharge = RechargeRecord.objects.create(customer_id=user_id, out_trade_no=out_trade_no, charge_mode_id=1, charge_money=total_amount, charge_date=charge_time, pay_status=2, charge_detail='')
    ...
    def post(self,request):
        ...
        if AlipayAPI().CheckAppPay(result):
            ...
            if result['trade_status'] == 'WAIT_BUYER_PAY':
                ...
                pay_status = 1
            elif result['trade_status'] == 'TRADE_SUCCESS':
                pay_status = 0
            elif result['trade_status'] == 'TRADE_FINISHED':
                charge_detail = str(result)
            try:
                recharge_list = RechargeRecord.objects.filter(out_trade_no=result['out_trade_no']).update(pay_status=pay_status, pay_date=result['gmt_payment'],pay_money=result['buyer_pay_amount'] ,charge_detail=charge_detail)

            except ObjectDoesNotExist as e:
                logger.error(e)

                Wallet.objects.filter(customer_id=recharge_list.customer).update(.88)
